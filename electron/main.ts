...
const PIPE_NAME = process.platform === 'win32'
  ? '\\\\.\\pipe\\speaky-single-instance-pipe'
  : path.join(os.tmpdir(), 'speaky-single-instance.sock');
...
function applyAutoStart(enabled: boolean) {
  try {
    ...
    if (isPackaged) {
      ...
      const exePath = path.join(process.cwd(), 'node_modules', 'electron', 'dist', 'speaky.exe');
