...
class StorageService {
  ...
  constructor() {
    let userDataPath = process.env.APPDATA ? path.join(process.env.APPDATA, 'speaky') : process.cwd();
    if (app?.getPath) {
      try {
        const appPath = app.getPath('userData');
        if (fs.existsSync(path.join(appPath, 'speaky-data.json'))) {
    ...
    this.filePath = path.join(userDataPath, 'speaky-data.json');
