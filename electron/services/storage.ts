  ...
  private resolveFilePath(): string {
    ...
    if (!dir) {
      if (process.platform === 'win32' && process.env.APPDATA) {
        dir = path.join(process.env.APPDATA, 'speaky');
      } else if (process.platform === 'darwin') {
        const home = process.env.HOME || '';
        dir = path.join(home, 'Library', 'Application Support', 'speaky');
      } else {
        const home = process.env.HOME || '';
        dir = path.join(home, '.config', 'speaky');
      }
    }
    ...
    return path.join(dir, 'speaky-data.json');
  }
