...
export const FloatingHud: React.FC = () => {
  ...
  const [isVisible, setIsVisible] = useState<boolean>(() => typeof window !== 'undefined' && !window.speakyAPI);
  ...
  const scheduleDismiss = (delayMs = 1400) => {
    ...
    if (typeof window !== 'undefined' && !window.speakyAPI) {
      return;
    }
  ...
  useEffect(() => {
    ...
    window.speakyAPI.getActiveContext().then((ctx: ActiveContext) => {
      if (ctx) setContext(ctx);
    });

    window.speakyAPI.getSettings?.().then((s: any) => {
      if (s) {
    ...
    const unsubSettings = window.speakyAPI.onSettingsChanged?.((s: any) => {
      if (s) {
        if (s.language) setCurrentLanguage(s.language);
        if (s.uiLanguage) setUiLanguage(s.uiLanguage);
        if (s.mode) setAppMode(s.mode);
    ...
    const unsubContext = window.speakyAPI.onContextChanged((newCtx: ActiveContext) => {
      setContext(newCtx);
    });

    const unsubSelection = window.speakyAPI.onSelectionChanged?.((data: any) => {
      if (data) {
  ...
  const handleDragStart = (e: React.MouseEvent) => {
    ...
    const handleMouseMove = (moveEvent: MouseEvent) => {
      ...
      window.speakyAPI?.moveHud?.(deltaX, deltaY);
    };
