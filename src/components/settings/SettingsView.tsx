...
export const SettingsView: React.FC = () => {
  ...
  useEffect(() => {
    ...
    window.speakyAPI.getDictionary().then((d: CustomWord[]) => d && setDictionary(d));
    ...
    const unsubSnippets = (window.speakyAPI as any)?.onSnippetsChanged?.((sn: TextSnippet[]) => {
  ...
  const handleSaveDictionary = async (dict: CustomWord[]) => {
    setDictionary(dict);
    if (window.speakyAPI) {
      await window.speakyAPI.saveDictionary(dict);
    }
  };
